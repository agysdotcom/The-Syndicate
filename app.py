...
def fetch_football_fixtures(league_id: int, date_iso: str) -> List[Dict]:
    url = f"{BASE_FOOTBALL}/fixtures"
    params = {"league": league_id, "season": 2025, "date": date_iso}
    try:
...
with tab[2]:  # Today
    ...
    for lid in LEAGUES.values():
        all_fixtures += fetch_football_fixtures(lid, date_today)
    render_fixtures(all_fixtures, date_today)

with tab[3]:  # Tomorrow
    all_fixtures = []
    for lid in LEAGUES.values():
        all_fixtures += fetch_football_fixtures(lid, date_tomorrow)
