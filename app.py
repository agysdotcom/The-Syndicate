...
def poisson_cdf_over(total, lam_total): return 1-sum(poisson_pmf(k, lam_total) for k in range(int(math.floor(total+0.5))))
...
def generate_betslips(fixtures:List[Dict])->Dict[str,List[Dict]]:
    ...
    for f in fixtures:
        ...
        pick=None
        if f["home_prob"]==max_win: pick=f"Home Win ({f['home']})"
        elif f["away_prob"]==max_win: pick=f"Away Win ({f['away']})"
        if max_win>conf_thresh and pick: winners.append({"match":f"{f['home']} vs {f['away']}","pick":pick,"prob":max_win,"confidence":f["confidence"],"game":f})
        for k,v in f["over_under"].items():
            if k.lower().startswith("over") and v>conf_thresh: goals.append({"match":f"{f['home']} vs {f['away']}","pick":k,"prob":v,"confidence":f["confidence"],"game":f})
        if sum(f.get("yellow_cards",{"home":0,"away":0}).values())>=3: yellow.append({"match":f"{f['home']} vs {f['away']}","pick":"High Yellow Cards","prob":None,"confidence":"N/A","game":f})
        if sum(f.get("corners",{"home":0,"away":0}).values())>=6: corners.append({"match":f"{f['home']} vs {f['away']}","pick":"High Corners","prob":None,"confidence":"N/A","game":f})
...
if league_selected=="All":
    for lid in LEAGUES.values():
        if lid: fixtures.extend(prepare_fixtures_with_stats(lid,date_today))
else:
...
if league_names:
    ...
    for idx,lname in enumerate(league_names):
        with league_tabs[idx]:
            ...
            if not league_fixtures: st.info(f"No matches available for {lname}")
